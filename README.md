...
## 🧠 Step 4.5: Memory Setup (Optional but Recommended)

Bot ko pichli baatein yaad rakhne ke liye [Upstash](https://upstash.com) (free Redis service) use karte hain:

1. [upstash.com](https://upstash.com) par free account banao
2. **"Create Database"** → naam do → region select karo (jo Vercel function ke region ke kareeb ho)
3. Database open karo → **"REST API"** section me ye 2 values milengi:
   - `UPSTASH_REDIS_REST_URL`
   - `UPSTASH_REDIS_REST_TOKEN`
4. Inhe Vercel env vars me add karo (Step 4 wale table me already list hain)

> Agar ye env vars set nahi karoge, bot phir bhi kaam karega — bas har message ko bina pichli context ke treat karega.
...
3. **Environment Variables** add karo (Settings → Environment Variables):
   ...
   | `UPSTASH_REDIS_REST_URL`     | (optional, memory ke liye)          |
   | `UPSTASH_REDIS_REST_TOKEN`   | (optional, memory ke liye)          |
...
- `/start`, `/help`, `/model` (current model dekho), `/clear` (memory bhula do) — sab handled hain.
- Bot ki personality ek "savage masti-khor dost" jaisi hai (halka-fulka taana/gaali, Hinglish), aur Upstash set hone par pichli baatein yaad rakhta hai.
